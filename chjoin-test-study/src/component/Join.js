...
//useRef : HTML의 특정 요소에 접근하기 위한 ID 설정
//뷰를 선택하는 도구.

//프로필 사진을 파일에서 선택 후 , 결과 뷰를 보여주기
// antd : 라이브러리, -> 버튼 뷰, 아바타 뷰, 샘플을 가져와서 사용
//컴포넌트 (레고블록)
//yarn add antd (npm install antd ) 설치가 우선 필요함.
...
const Join = () => {
  ...
  //파일 세팅. 선택된 파일을 변경하는 함수
  const [File, setFile] = useState("");

  //input태그에 접근하기 위한 ref속성달기 -> 뷰에 접근하기 위해 사용.
  ...
  const onChangeImage = (e) => {
    ...
    if (e.target.files[0]) {
      ...
      //취소가 발생했다면, 기본 프로필 베이직 사진으로 설정 하겠다.
      setImage(
        "https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_1280.png"
      );
      return;
